...
import { changeIdentityToUser,searchUser,
...
const getAdminglistLength = async ()=>{
  const res = await getAdmingListLength("产品管理员")
...
const getFirstPageList = async ()=>{
  ...
  getAdminglistLength()
  const res = await returnListData("1",0,'产品管理员')
  if(res != null){
    const oid = res.oid
...
const currentchange = async (value:number) =>{
  ...
  const res = await returnListData(klope.value,0,'产品管理员')
  if(res != null){
    const oid = res.oid
    tableData.value = res.result
    zhuihoid.value[Math.ceil(paginationData.currentPage - 1)] = res.result[oid].id
