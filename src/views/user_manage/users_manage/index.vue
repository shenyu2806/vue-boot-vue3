    ...
    <div class="table-top">
      <div class="table-header">
        ...
        <div class="button-wrapped">
          <el-button type="primary" @click="openCreate(3)">添加用户管理员</el-button>
        </div>
      </div>
        ...
        <el-table :data="tableData" border  style="width: 100%" @row-dblclick="openuse">
          ...
          <el-table-column prop="update_time" label="更新时间" align="center">
            <template #default="{row}">
              <div>{{ row.update_time?.slice(0,16) }}</div>
            </template>
          </el-table-column>
...
const getAdminglistLength = async ()=>{
  const res = await getAdmingListLength("用户管理员")
...
const getFirstPageList = async ()=>{
  ...
  getAdminglistLength()
  const res = await returnListData("1",0,'用户管理员')
  if(res != null){
    const oid = res.oid
...
const currentchange = async (value:number) =>{
  ...
  const res = await returnListData(klope.value,0,'用户管理员')
  if(res != null){
    const oid = res.oid
    tableData.value = res.result
    zhuihoid.value[Math.ceil(paginationData.currentPage - 1)] = res.result[oid].id
