...
// changecompanyintro
import { changeCompanyIntroduce, getCompanyIntroduce} from '@/api/stting'
const title = ref()
bus.on("editorTitle", async (id : number) => {
  ...
  if (id == 4) {
    title.value = '编辑公司现任高层'
    valueHtml.value = await getCompanyIntroduce('公司现任高层')
  }
})
...
const yes = async () => {
  // 去除 编辑两个字
  let titlede = title.value.slice(-4);
  if(title.value=='编辑公司现任高层'){
    titlede = title.value.slice(-6);
  }
  // 两个参数 set_text set_name
  const res = await changeCompanyIntroduce(valueHtml.value,titlede)
  if (res.status == 0) {
    ElMessage({
      message: res.message,
      type: 'success',
