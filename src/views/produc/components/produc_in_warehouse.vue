<template>
  <el-dialog v-model="dialogVisible" title="添加产品入库" width="600px" align-center draggable>
    <div class="dialog-content">
      <el-form ref="ruleForDataRef" style="max-width: 600px" :model="ruleForData" :rules="rules" label-width="100px">
        <el-form-item label="入库编号" prop="account">
          <el-input v-model="ruleForData.product_id" />
        </el-form-item>
...
import { createProduct } from '@/api/produc'
...
const addAdmin = async () =>{
  ...
  const res = await createProduct(ruleForData)
  if(res.status==0){
    ElMessage({
      message: "入库成功！请刷新页面",
      type: 'success',
